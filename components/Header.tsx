    ...
    <header className={`bg-white/90 backdrop-blur-md border-b border-slate-200 sticky z-40 shadow-soft transition-all duration-300 ease-out ${showAppInstallBanner ? 'top-16 sm:top-20' : 'top-0'}`}>
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        <div className="flex justify-between items-center h-14 sm:h-16">
          {/* Logo and Title - Mejorado para móvil */}
          <div className="flex items-center space-x-2 sm:space-x-4">
            <button
              onClick={onNavigateHome}
              className="flex items-center space-x-2 sm:space-x-3 group hover:opacity-80 transition-opacity duration-200 touch-friendly"
            >
              ...
              <div className="sm:hidden">
                <h1 className="text-base sm:text-lg font-bold text-slate-800 group-hover:text-ocean-600 transition-colors duration-200">
                  {title}
                </h1>
              </div>
            </button>
          ...
          {/* Navigation - Unificada para todos los tamaños */}
          <nav className="flex items-center space-x-1 sm:space-x-2">
            <Button
              onClick={onNavigateHome}
              variant={currentView === AppView.MAIN_APP ? "primary" : "secondary"}
              size="sm"
              className={`px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 rounded-lg transition-all duration-200 ${
            ...
            >
              <span className="flex items-center gap-1 sm:gap-2">
                <span>📝</span>
                <span className="hidden sm:inline lg:inline">Blog</span>
              </span>
            </Button>
          </nav>

          {/* Status Indicator - Mejorado para móvil */}
